                 ...
                 if(err){
                    ...
                    pdf.create(results).toFile('./generated_pdf/'+generateUuid + ".pdf", function(err,data){
                        if(err){
                            ...
                            return res.status(200).json({status:true,uuid:generateUuid});
                        }
                    })
                 }
            })
...
router.get('/getBills', auth.authenticateToken , (req,res, next)=>{
    ...
    connection.query(query, (err, results)=>{
        if(!err){
            return res.status(200).json({status:true, bills:results});
        }
        else{
            return res.status(500).json({status:false,message:err});
        }
...
router.delete('/delete/:id', auth.authenticateToken , (req, res)=>{
    ...
    connection.query(query , [id], (err,results)=>{
        if(!err){
            ...
            return res.status(200).json({status:true, message:"Bill deleted Successfully"});
        }
        else{
            return res.status(500).json({status:false,message:err});
        }
